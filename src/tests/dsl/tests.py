...
class DslTest(TestCase):
    def setUp(self):
        ...
        self.event_validation_v7 = re.findall('__EVENTVALIDATION.*?value=\"(.*?)\"', response_v7.content)
        self.view_state_v7 = re.findall('__VIEWSTATE\".*?value=\"(.*?)\"', response_v7.content)

        response_v8 = requests.get('https://pqcc.soap.dslorder.nl/pqcc/v8.0/pqcc.aspx')
        self.event_validation_v8 = re.findall('__EVENTVALIDATION.*?value=\"(.*?)\"', response_v8.content)
        self.view_state_v8 = re.findall('__VIEWSTATE\".*?value=\"(.*?)\"', response_v8.content)
